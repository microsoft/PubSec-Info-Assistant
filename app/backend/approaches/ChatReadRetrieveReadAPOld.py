...
class LangchainPromptTemplates(Enum):
    SystemMessagePromptTemplate = """system{persona}
...
"""
    ...
    HumanMessagePromptTemplate = """human{persona}
{injected_prompt}
{message}
"""

    AIMessagePromptTemplate = """assistant{persona}
{injected_prompt}
{message}
"""
...
"""
    ...
    def run(self, history: list[dict], overrides: dict) -> any:
        ...
        if prompt_override is None:
            prompt = self.prompt_prefix.format(persona="", injected_prompt="", sources=content, chat_history=self.get_chat_history_as_text(history), follow_up_questions_prompt=follow_up_questions_prompt)
        elif prompt_override.startswith(">>>"):
            prompt = self.prompt_prefix.format(persona="", injected_prompt=prompt_override[3:] + "\n ", sources=content, chat_history=self.get_chat_history_as_text(history), follow_up_questions_prompt=follow_up_questions_prompt)
        else:
            prompt = prompt_override.format(persona="", sources=content, chat_history=self.get_chat_history_as_text(history), follow_up_questions_prompt=follow_up_questions_prompt)

        # Add persona to the prompt
        prompt += LangchainPromptTemplates.HumanMessagePromptTemplate.value.format(persona=userPersona, injected_prompt="", message="")
    ...
    def get_chat_history_as_text(self, history, include_last_turn=True, approx_max_tokens=1000) -> str:
        ...
        for h in reversed(history if include_last_turn else history[:-1]):
            history_text = """human{persona}""".format(persona=h.get("persona") + " ") + "\n" + """user""" +"\n" + h["user"] + "\n" + """assistant""" + "\n" + (h.get("bot") + """""" if h.get("bot") else "") + "\n" + history_text
