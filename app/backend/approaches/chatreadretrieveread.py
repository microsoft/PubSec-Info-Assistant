...
class ChatReadRetrieveReadApproach(Approach):
    ...
    def __init__(
        ...
        self.log, self.tracer, self.meter = configure_monitoring()
    ...
    async def run(self, history: Sequence[dict[str, str]], overrides: dict[str, Any], citation_lookup: dict[str, Any], thought_chain: dict[str, Any]) -> Any:
        ...
        try:
            ...
            self.log.error(f"Error generating optimized keyword search: {str(e.body['message'])}")
            yield json.dumps({"error": f"Error generating optimized keyword search: {str(e.body['message'])}"}) + "\n"
            return
        except Exception as e:
            self.log.error(f"Error generating optimized keyword search: {str(e)}")
        ...
        try:
            ...
            if response and response.data:
                ...
                self.log.info(f"Generated embeddings")
            else:
                # Generate an error message if the embedding generation fails
                self.log.error(f"Error generating embedding:: {response.status_code} - {response.text}")
            ...
            self.log.error(f"Error generating embedding: {str(e)}")
        ...
        for idx, doc in enumerate(r):  # for each document in the search results
            if doc.get(self.source_file_field) is not None:
                ...
                # Handle the case where the source file field is None
                self.log.warning(f"Warning: Document {idx} has no source file field.")
            ...
            if doc["file_uri"] is not None:
                parsed_url = urllib.parse.urlparse(doc["file_uri"])
                path_parts = parsed_url.path.split('/')
                desired_part = '/'.join(path_parts[path_parts.index('upload'):])
                page = self.extract_page_number(doc['id'])
                citation_lookup[f"File{idx}"] = {
                    "citation": doc["file_uri"],
                    "source_path": desired_part,
                    "page_number": page or "0"
                }
            # parsed_url = urllib.parse.urlparse(doc["file_uri"])
            # path_parts = parsed_url.path.split('/')
            # desired_part = '/'.join(path_parts[path_parts.index('upload') + 1:])
            # page = self.extract_page_number(doc['id'])
            # citation_lookup[f"File{idx}"] = {
            #     "citation": doc["file_uri"],
            #     "source_path": desired_part,
        ...
        try:
            ...
            self.log.error(f"Error generating chat completion: {str(e.body['message'])}")
            yield json.dumps({"error": f"Error generating chat completion: {str(e.body['message'])}"}) + "\n"
            return
        except Exception as e:
            self.log.error(f"Error generating chat completion: {str(e)}")
    ...
    def get_source_file_with_sas(self, source_file: str) -> str:
        ...
        try:
            ...
            self.log.error(f"Unable to parse source file name: {str(error)}")
            return ""
