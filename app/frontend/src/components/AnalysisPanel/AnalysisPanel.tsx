...
export const AnalysisPanel = ({ answer, activeTab, activeCitation, sourceFile, pageNumber, citationHeight, className, onActiveTabChanged }: Props) => {
  ...
  const onRenderItemLink =
    (content: string | JSX.Element | JSX.Element[] | undefined, tooltipRef: IRefObject<ITooltipHost> | undefined, shouldRender: boolean) =>
    (properties: IPivotItemProps | undefined, nullableDefaultRenderer?: (props: IPivotItemProps) => JSX.Element | null) => {
    ...
    >
      ...
      <PivotItem
        ...
        onRenderItemLink={onRenderItemLink(
          "No active citation selected. Please select a citation from the citations list on the left.",
          tooltipRef3,
        ...
        >
          ...
          <PivotItem itemKey="rawFile" headerText="Document">
            {getCitationURL() === "" ? (
              ...
              <iframe
                title="Source File"
                src={"https://view.officeapps.live.com/op/view.aspx?src=" + encodeURIComponent(getCitationURL()) + "&action=embedview&wdStartOn=" + pageNumber}
                width="100%"
