    ...
    return (
        <Stack className={`${answer.approach == Approaches.ReadRetrieveRead ? styles.answerContainerWork : 
                ...
                <Stack.Item>
                    <Stack horizontal wrap tokens={{ childrenGap: 5 }}>
                        ...
                        {parsedAnswer.web_citations.map((x, i) => {
                            ...
                            return (
                                <a key={i} className={styles.citationWeb} tabIndex={0}
                ...
                <Stack.Item>
                    <Stack horizontal wrap tokens={{ childrenGap: 5 }}>
                        ...
                        {parsedAnswer.work_citations.map((x, i) => {
                            const path = getCitationFilePath(x);
                            return ( 
                                 <a key={i} className={styles.citationWork} tabIndex={0}
                ...
                <Stack.Item>
                    <Stack horizontal wrap className={`${!!parsedAnswer.work_citations.length ? styles.followupQuestionsList : !!parsedAnswer.web_citations.length ? styles.followupQuestionsList : ""}`} tokens={{ childrenGap: 6 }}>
                        ...
                        {parsedAnswer.followupQuestions.map((x, i) => {
                            return (
                                <a key={i} tabIndex={0} className={styles.followupQuestion} title={x} onClick={() => onFollowupQuestionClicked(x)}>
                                    {`${x}`}
                                </a>
                            );
                        })}
            ...
            {answer.answer && <Stack.Item align="center">
                <RAIPanel tabIndex={0} approach={answer.approach} chatMode={chatMode} onAdjustClick={onAdjustClick} onRegenerateClick={onRegenerateClick} onWebSearchClicked={onWebSearchClicked} onWebCompareClicked={onWebCompareClicked} onRagCompareClicked={onRagCompareClicked} onRagSearchClicked={onRagSearchClicked} />
            </Stack.Item>}
        </Stack>
    );
};
