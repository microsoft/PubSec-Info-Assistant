...
export function parseAnswerToHtml(
  ...
  onCitationClicked: (citationFilePath: string, citationSourcePath: string, pageNumber: string) => void
): HtmlParsedAnswer {
        ...
        if (approach == Approaches.ReadRetrieveRead) {
          ...
          if (!citation) {
            ...
            return renderToStaticMarkup(
              <a
                className={styles.supContainerWork}
                title={citation.citation.split("/").slice(4).join("/")}
                onClick={() => onCitationClicked(path, sourcePath, pageNumber)}
              >
