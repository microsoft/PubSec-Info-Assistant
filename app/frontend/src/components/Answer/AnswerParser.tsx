            ...
            if (!pattern.test(part)) {
                ...
                if (approach == Approaches.ReadRetrieveRead) {
                    ...
                    if (!citation) {
                        ...
                        const path = citation.citation;
                        const sourcePath = citation.citation
        ...
        work_fragments = work_parts.map((part, index) => {
            ...
            if (index % 2 === 0) {
                ...
                if (!work_citation) {
                    ...
                    const citationShortName = work_citation.citation.split("/").slice(4).join("/");
                    // Check if the citation is already in the citations array
                    if (work_citations.includes(citationShortName)) {
                        // If it exists, don't add it again
                    } else {
                        work_citations.push(citationShortName);
                        work_sourceFiles[citationShortName] = work_citation.source_path;
                        pageNumbers[citationShortName] = Number(work_citation.page_number);
                    }
                }
            }
            return "";
