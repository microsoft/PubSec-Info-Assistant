...
const CharacterStreamer = ({ eventSource, nonEventString, onStreamingComplete, classNames }: { eventSource?: any; nonEventString?: string, onStreamingComplete: any; classNames?: string }) => {
  ...
  useEffect(() => {
    ...
    const handleMessage = async (event: MessageEvent) => {
        ...
        queueRef.current = queueRef.current.concat(event.data.split(''));
  ...
  const processQueue = () => {
    ...
    }, 40); // Adjust based on desired "typing" speed
  };
