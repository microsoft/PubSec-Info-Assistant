...
export const FileStatus = ({ className }: Props) => {
    ...
    function convertStatusToItems(fileList: FileUploadBasicStatus[]) {
        ...
        for (let i = 0; i < fileList.length; i++) {
            ...
            const filePath = fileList[i].file_path;
            try {
                items.push({
                    key: fileList[i].id,
                    ...
                    filePath: filePath,
                    fileFolder: filePath === 'upload' ? 'upload' : filePath.split('/').pop() || '',
                    ...
                    status_updates: Array.isArray(fileList[i].status_updates) ? fileList[i].status_updates : [],
                    value: fileList[i].id,
