    ...
    if (!allowNewFolders) {
      if (option) {
        if (option?.itemType === SelectableOptionMenuItemType.SelectAll) {
          ...
          const updatedKeys = selected ? [...currentSelectedOptionKeys, option!.key as string] : currentSelectedOptionKeys.filter(k => k !== option.key);
  ...
  return (
    <div className={hide ? styles.hide : styles.folderArea}>
      <div className={styles.folderSelection}>
        ...
        />
        <TooltipHost
          content={allowNewFolders ? "Select a folder to upload documents into" : "Select a folder to filter the search by"}
          styles={hostStyles}
          id={tooltipId}
        >
