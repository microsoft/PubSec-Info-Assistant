...
const Tda = ({ folderPath, tags }: Props) => {
      ...
      files.forEach(async (indexedFile: any) => {
        ...
        Papa.parse(file, {
          ...
          complete: async function (results) {
            ...
            try {
              const response = await postTd(file)
                .then(response => {
                  setProgress(100);
  ...
  const columnLengths: { [key: string]: number } = dataFrame.reduce(
    (lengths: { [key: string]: number }, row: Record<string, any>) => {
    ...
    Object.keys(dataFrame[0] || {}).reduce(
      (lengths: { [key: string]: number }, key: string) => {
  ...
  return (
    <div className={cstyle.contentArea}>
      <div className={cstyle.App}>
        ...
        <h1 className={cstyle.EmptyStateTitle}>Tabular Data Assistant</h1>
        <span className={styles.chatEmptyObjectives}>
          <i className={cstyle.centertext}>Information Assistant uses AI. Check for mistakes.</i>{" "}
        ...
        <div className={cstyle.centeredContainer}>
          ...
          <span className={cstyle.EmptyObjectivesListItemText}>Max file size: {maxCSVFileSize?.MAX_CSV_FILE_SIZE} MB</span>
          <br />
          <div className={styles.wrapper}>
            ...
            {files.length ? (
              <button
                ...
                className={classNames(styles.upload_button, uploadComplete || uploadStarted ? styles.disabled : "")}
          ...
          <div>
            ...
            <div>
              ...
              <div className={cstyle.buttonContainer}>
                <Button variant="secondary" onClick={handleAnalysis}>
                  Here is my analysis
                </Button>
                <Button variant="secondary" onClick={handleAnswer}>
                  Show me the answer
                </Button>
              </div>
              {
                  ...
                  <div>
                    {renderAnswer && (
                      <CharacterStreamer
                        ...
                        classNames={cstyle.centeredAnswerContainer}
                  ...
                  <div>
                    {images.length > 0 ? (
                      images.map((image, index) => (
                        <img key={index} src={`data:image/png;base64,${image}`} alt={`Temp Image ${index}`} style={{ maxWidth: "100%" }} />
                      ))
                    ) : (
