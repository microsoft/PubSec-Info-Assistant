...
module "backend" {
  ...
  appSettings = {
    ...
    AZURE_CLIENT_ID = var.isInAutomation ? var.aadMgmtClientId : module.entraRoles[0].azure_ad_mgmt_app_client_id 
    AZURE_CLIENT_SECRET = var.isInAutomation ? var.aadMgmtClientSecret : module.entraRoles[0].azure_ad_mgmt_app_secret 
  ...
  aadClientId = var.isInAutomation ? var.aadWebClientId :module.entraRoles[0].azure_ad_web_app_client_id
  depends_on = [ module.kvModule ]
}
...
module "openAiRoleMgmt" {
  ...
  principalId     = var.isInAutomation ? var.principalId : module.entraRoles[0].azure_ad_mgmt_sp_id 
  roleDefinitionId = local.azure_roles.CognitiveServicesOpenAIUser
  principalType   = "ServicePrincipal"
  subscriptionId   = data.azurerm_client_config.current.subscription_id
  resourceGroupId  = azurerm_resource_group.rg.id
...
module "kvModule" {
  ...
  spClientSecret    = var.isInAutomation ? var.aadMgmtClientSecret : module.entraRoles[0].azure_ad_mgmt_app_secret 
