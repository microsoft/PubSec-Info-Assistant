locals {
  arm_file_path = var.useDDOSProtectionPlan ? "arm_templates/network/vnet_w_ddos.template.json" : "arm_templates/network/vnet.template.json"
}
...
resource "azurerm_network_ddos_protection_plan" "ddos" {
  count               = var.useDDOSProtectionPlan ? var.ddos_plan_id == "" ? 1 : 0 : 0
  name                = var.ddos_name
  resource_group_name = var.resourceGroupName
  location            = var.location
} 
...
resource "azurerm_resource_group_template_deployment" "vnet_w_subnets" {
  ...
  parameters_content = jsonencode({
    ...
    "tags"                      = { value = var.tags },
    "ddos_plan_id"              = { value = "${var.useDDOSProtectionPlan ? var.ddos_plan_id == "" ? azurerm_network_ddos_protection_plan.ddos[0].id : var.ddos_plan_id : ""}" },
    ...
    "subnet_Bing_CIDR"          = { value = "${var.snetBingServiceCIDR}" },
