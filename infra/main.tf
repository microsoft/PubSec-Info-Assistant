...
// Create the newtwork security perimiter
...
module "storage" {
  ...
  network_rules_allowed_subnets   = [module.network.snetIntegration_id]
...
module "kvModule" {
  ...
  arm_template_schema_mgmt_api  = var.arm_template_schema_mgmt_api
  deployment_machine_ip         = var.deployment_public_ip
  bing_secret_value             = module.bingSearch[0].key
...
module "webapp" {
  ...
  scm_public_ip                       = var.deployment_public_ip
  randomString                        = random_string.random.result
  azure_environment                   = var.azure_environment
...
module "searchServices" {
  ...
  deployment_public_ip            = var.deployment_public_ip
...
module "openAiRoleMgmt" {
  ...
  count = var.existingAzureOpenAIResourceGroup == "" ? 1 : var.isInAutomation ? 0 : 1
  scope = var.existingAzureOpenAIResourceGroup == "" ? azurerm_resource_group.rg.id : data.azurerm_resource_group.existing[0].id
