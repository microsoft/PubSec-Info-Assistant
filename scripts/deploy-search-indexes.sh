...
response=$(curl -s --show-error -w "http_code=%{http_code}" -X PUT --header "Content-Type: application/json" --header "Authorization: Bearer $access_token" --data "$skillset_json" "$search_url/skillsets/$skillset_name?api-version=2024-11-01-Preview")
displayError
