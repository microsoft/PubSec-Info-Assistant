...
if [ -n "${IN_AUTOMATION}" ]; then
  ...
  aadWebAppId=$(az ad app list --display-name infoasst_web_access_quawn --output tsv --query [].appId)
  if [ -z $aadWebAppId ]
    then
  ...
  aadWebSPId=$(az ad sp list --display-name infoasst_web_access_quawn --output tsv --query "[].id")
  ...
  aadMgmtAppId=$(az ad app list --display-name infoasst_mgmt_access_quawn --output tsv --query [].appId)
  if [ -z $aadMgmtAppId ]
    then
    ...
    aadMgmtAppSecret="w5Q8Q~T12POZhnM40FmtKJwa6IUMuM7Cbsh0-anN"
    echo "Secret Reset"

  #aadMgmtSPId=$(az ad sp list --display-name infoasst_mgmt_access_$RANDOM_STRING --output tsv --query "[].id")
  aadMgmtSPId=$(az ad sp list --display-name infoasst_mgmt_access_quawn --output tsv --query "[].id")
