...
if [ "$FROM_PIPELINE" = "true" ]; then
    ...
    while [ $count -lt $retries ]; do
        ...
        echo "deployment machine public ip with ipinfo is" $ip

        if [ -z "$deployment_machine_ip" ]; then
            deployment_machine_ip="$ip"
        elif ! echo "$deployment_machine_ip" | grep -q "$ip"; then
            deployment_machine_ip="$deployment_machine_ip $ip"
        fi

        count=$((count+1))
    done

    deployment_machine_ip=$(echo $deployment_machine_ip | jq -R 'split(" ")' | jq -s '.')
    echo "All distinct deployment machine public IPs: $deployment_machine_ip"
...
azd env set DEPLOYMENT_PUBLIC_IP "$deployment_public_ip"
azd env get-values | grep DEPLOYMENT_PUBLIC_IP

echo "deployment machine public ip is" $deployment_public_ip
