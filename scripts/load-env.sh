...
if [ "$USE_CUSTOM_ENTRA_OBJECTS" = "true" ]; then  
    ...
    azd env set AAD_WEB_CLIENT_ID "$aadWebAppId"
    azd env set AAD_MGMT_CLIENT_ID "$aadMgmtAppId"
    azd env set AAD_MGMT_SP_ID "$aadMgmtSPId"
fi
